...
class HFLensModel:
    ...
    def __init__(
        ...
        *,
        ...
        self.layers: nn.ModuleList = getattr(self._text_module, layout.layers) # Residual block
        self._final_norm: nn.Module = getattr(self._text_module, layout.norm) # Final layer normalization
        self._embed_tokens: nn.Module = getattr(self._text_module, layout.embed) # input embeddings.
        self._lm_head: nn.Module = getattr(hf_model, layout.lm_head) # output projection to vocabulary.
...
"""
                    ...
                    encode()      forward()    unembed()
                        │             │             │
                        ▼             ▼             ▼
                Token IDs    Residual Stream   Logits
                                    │
                                    ▼
                            ActivationRecorder
                                    │
...
"""
Notice how hf.py doesn't implement the Jacobian algorithm at all. Its sole responsibility is to adapt diverse Hugging Face models into the uniform LensModel interface that the rest of the library expects.
